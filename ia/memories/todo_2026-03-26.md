...
- ✅ Terminé : -
- **Composants** :
    - `SB_WaveController` : (Ordonnancement des vagues)
    - `Formations` : (V_SHAPE, LINE, CLUSTER)
- **Objectif** : Structurer le niveau par vagues plutôt que par spawn aléatoire.
...
- [x] **Visuels** : Implémentation des fragments en cubes colorés (Énergie: Jaune, Bouclier: Bleu, Éther: Blanc). ✅
- [x] **Drops Fixes** : Mise à jour de `SB_Enemy_VShmup.gd` pour lâcher des quantités précises de fragments à la mort. ✅
- [x] **Éther Quantique** : Intégration de la monnaie dans le système de stats du Core (`add_ether`). ✅
