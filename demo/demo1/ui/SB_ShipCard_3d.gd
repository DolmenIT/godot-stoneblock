...
# ── Matériaux ────────────────────────────────────────────────
var _mat_socle: StandardMaterial3D
var _mat_ship: StandardMaterial3D
var _mat_deco: StandardMaterial3D
...
func _create_ui_material(mesh: MeshInstance3D) -> StandardMaterial3D:
...
func _update_card() -> void:
	...
	# --- Layer 0 : Socle ---
	if _mat_socle:
		_mat_socle.albedo_texture = ship_socle_texture
		_mat_socle.emission_texture = ship_socle_texture
	if _mesh_socle:
		...
		_mesh_socle.layers = 2049 if bloom_socle else 1

	# --- Layer 1 : Ship ---
	if _mat_ship:
		_mat_ship.albedo_texture = ship_texture
		_mat_ship.emission_texture = ship_texture
	if _mesh_ship:
		...
		_mesh_ship.layers = 2049 if bloom_ship else 1
	...
	if _mat_deco:
		_mat_deco.albedo_texture = final_deco_tex
		_mat_deco.emission_texture = final_deco_tex
	if _mesh_deco:
		...
		_mesh_deco.layers = 2049 if bloom_deco else 1
