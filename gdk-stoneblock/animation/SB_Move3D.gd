...
func play_sequence() -> void:
	...
	for step in steps:
		...
		# SÃ©quenÃ§age prÃ©cis via parallel() et set_delay()
...
func _process(delta: float) -> void:
	if Engine.is_editor_hint():
		...
		# --- play_in_editor : tout gÃ©rÃ© manuellement, sans dÃ©pendre du Tween ---
		...
		return
	# --- Mode jeu : Tween gÃ¨re la position, _process gÃ¨re l'orientation ---
	if target_node:
...
## Avance sur la courbe depuis [t_start] jusqu'Ã  atteindre [target_dist] en unitÃ©s monde.
## Retourne le t correspondant. RÃ©solution : [num_steps] Ã©chantillons max.
func _find_t_at_distance(step: SB_MoveStep, t_start: float, target_dist: float, num_steps: int = 64) -> float:
	...
	for i in range(1, num_steps + 1):
		...
		if accumulated >= target_dist:
			# Interpolation linÃ©aire pour affiner le t exact
...
## Identique Ã  _find_t_at_distance mais VERS L'ARRIÃˆRE depuis t_start.
...
func _update_node_orientation(step: SB_MoveStep, t: float, delta: float, instant: bool = false) -> void:
	...
	# --- 1. Calculer p_a et p_b Ã  look_ahead_distance mÃ¨tres d'Ã©cart en espace LOCAL ---
	...
	if t_ahead > t_cur + 1e-6:
		...
		# En bout de courbe (tâ‰ˆ1) : on regarde en arriÃ¨re
	...
	# --- 2. Vecteur "haut" exprimÃ© en espace parent-local ---
	# Les positions du step sont en espace local du parent â†’ la tangente aussi.
	# On ramÃ¨ne world-UP dans cet espace via la rotation inverse du parent (sans Ã©chelle).
	...
	# Basis.looking_at sur tangente locale â†’ q dÃ©jÃ  en local, prÃªt pour transform.basis.
	var q_trajectory: Quaternion = Basis.looking_at(tangent_local, up_local, step.use_model_front).get_rotation_quaternion()

	# --- 4. Appliquer le model_rotation_offset (cosmÃ©tique) ---
	...
	# --- 5. Appliquer sur transform.basis (local, stable Ã©diteur et jeu) ---
