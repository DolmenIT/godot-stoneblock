...
## Espacement temporel (en sec) entre les particules d'un burst pour créer une traînée.
...
func _update_particles(delta: float) -> void:
	...
	for p in _particle_pool:
		...
		# Tant que l'Ã¢ge est nÃ©gatif, la particule attend son tour Ã  l'origine
		if age < 0:
			p.visible = false
			continue
			
		# Au moment de l'activation (passage Ã  age >= 0)
		if not p.visible:
			p.visible = true
		...
		# Manuel Billboard : Force la particule Ã  faire face Ã  la camÃ©ra tout en prÃ©servant la rotation locale Z
		if billboard:
			var camera = get_viewport().get_camera_3d()
			if camera:
				# On aligne la basis de la particule sur celle de la camÃ©ra
				p.global_transform.basis = camera.global_transform.basis
				# Le scale sera rÃ©appliquÃ© proprement Ã  la fin de la fonction
		...
		if attraction_to_origin != 0:
			...
			if local_space:
				...
				# Si global, on rappelle que spawn_pos Ã©tait relatif Ã  l'Ã©metteur AU SPAWN
				# mais ici on veut peut-Ãªtre attirer vers le point global de spawn.
				# Pour simplifier StoneBlock, on attire vers le point GLOBAL calculÃ© au spawn.
