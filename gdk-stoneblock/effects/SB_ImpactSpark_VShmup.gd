...
## âœ¨ SB_ImpactSpark_VShmup : Petit Ã©clat lumineux Ã  l'impact des projectiles.
...
func _ready() -> void:
	...
	emitting = true
	# Auto-destruction aprÃ¨s l'Ã©mission
	var timer = get_tree().create_timer(lifetime + 0.1)
	timer.timeout.connect(queue_free)

func setup(color: Color) -> void:
	# Appliquer la couleur au matÃ©riau
	if draw_pass_1:
		...
		if mat is StandardMaterial3D:
			...
			new_mat.emission_energy_multiplier = 4.0 # TrÃ¨s lumineux
			draw_pass_1.surface_set_material(0, new_mat)
	
	# Optionnel : Ajouter une lumiÃ¨re omni trÃ¨s brÃ¨ve
