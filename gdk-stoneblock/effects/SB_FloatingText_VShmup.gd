...
## âœ‰ï¸ SB_FloatingText : Texte flottant (Version CamÃ©ra Orthographique).

@export_group("Animation Ã‰chelle")
...
# L'inclinaison vers la camÃ©ra (X) pour la lisibilitÃ© depuis une vue plongeante
...
func _ready() -> void:
	# Le texte est indÃ©pendant pour Ã©viter les problÃ¨mes de coordonnÃ©es
	top_level = true
	fixed_size = false
	render_priority = 100
	
	# Billboard dÃ©sactivÃ© pour permettre de pivoter manuellement sur l'axe Y
	billboard = BaseMaterial3D.BILLBOARD_DISABLED 
	
	# Inclinaison fixe vers la camÃ©ra sur l'axe X
	rotation_degrees.x = camera_tilt_degrees
	# On s'assure que Z (inclinaison latÃ©rale) reste Ã  zÃ©ro
	rotation_degrees.z = 0 
	
	scale = Vector3.ONE * min_scale
	modulate.a = 0.0

## Initialisation appelÃ©e par l'ennemi (Position & Texte)
...
## Calcul de la position et de la rotation sur l'axe Y Ã  chaque frame
