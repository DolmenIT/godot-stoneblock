...
## GÃ¨re le dÃ©coupage automatique, la segmentation par tuiles et le remplissage multi-directionnel.
...
## Si vrai, la barre est composÃ©e de morceaux individuels (tuiles).
...
## Si vrai, le remplissage est fluide (clippÃ©).
...
## Ã  sa taille de pixels initiale (multipliÃ©e par son propre scale local).
...
func _ready() -> void:
	# Forcer le filtre Pixel-Art (Nearest) pour Ã©viter le lissage
...
func _update_ignore_scale() -> void:
	var p = get_parent()
	if p is CanvasItem:
		# On rÃ©cupÃ¨re le scale global du parent (ex: scaling de Ã©cran/HUD)
		var p_scale = p.get_global_transform().get_scale()
		if p_scale.x != 0 and p_scale.y != 0:
			# On compense le scale du parent pour rester Ã  la taille de design
			self.scale = _design_scale / p_scale
			# Snapping de la position pour Ã©viter le flou de translation
...
func _draw_segmented(frame_count: int, ratio: float) -> void:
	...
	# 2. DÃ©finir la zone de progression globale (Safe Area)
	...
	# 3. Calculer le rectangle de remplissage actuel (clippÃ© par le ratio)
	...
	for i in range(frame_count):
		...
		if intersection.size == tile_rect.size:
			...
			# Source rect sur le sprite original (pour prÃ©lever le bon morceau)
			# On arrondi l'intersection pour Ã©viter les micro-lignes de bordure
...
func _draw_clipped_texture(tex: Texture2D, pos: Vector2, ratio: float) -> void:
	...
	# Utilisation de floor/round pour Ã©viter les interpolations et les micro-lignes
