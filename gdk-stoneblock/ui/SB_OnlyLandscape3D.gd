...
extends Node3D
class_name SB_OnlyLandscape3D

## 💻 SB_OnlyLandscape3D : Affiche ses enfants uniquement en mode Paysage.
## Si l'orientation est Portrait, le noeud devient invisible (visible = false) et se désactive (process_mode).
...
func _update_visibility() -> void:
	...
	if not Engine.is_editor_hint() and SB_Core.instance:
		is_landscape = SB_Core.instance.get_current_orientation() == SB_Core.SBOrientation.LANDSCAPE
	else:
	...
	if visible != is_landscape:
		visible = is_landscape
		process_mode = Node.PROCESS_MODE_INHERIT if is_landscape else Node.PROCESS_MODE_DISABLED
