...
## Note : Dans Godot, les marges dÃ©pendent du conteneur parent.
## Ces valeurs sont appliquÃ©es via custom_minimum_size ou injectÃ©es dans le parent.
...
func _update_layout() -> void:
	# On applique le padding via un override de StyleBox si on n'en a pas dÃ©jÃ  un du thÃ¨me
	# Mais le plus propre est de laisser le ThemeManager s'en occuper.
	# Ici, on fournit des overrides locaux si spÃ©cifiÃ©s.
