...
## Ce composant est dÃ©sormais centralisÃ© dans StoneBlock Core.
...
func _ready() -> void:
	# Connexion au redimensionnement de la fenÃªtre pour zoomer le HUD
...
func _process(_delta: float) -> void:
	...
	# Mise Ã  jour du Score (via GameMode)
	if gamemode and score_label:
		score_label.text = "SCORE: " + str(gamemode.get("score"))
	
	# Mise Ã  jour du Combo (DÃ©sactivÃ© au profit de l'IP-066 : Texte Flottant sur l'ennemi)
	if combo_label:
		combo_label.visible = false
	
	# Mise Ã  jour des Coins (via SB_Core)
	if coin_label and SB_Core.instance:
		var coins = SB_Core.instance.get_stat("magie")
		coin_label.text = "COINS: " + str(coins)
	elif coin_label:
		coin_label.text = "COINS: [N/A]" # Indique que le Core est manquant (test scÃ¨ne)
