...
const SB_BUTTON_SHADER = preload("res://gdk-stoneblock/ui/SB_Button.gdshader")
...
@export var normal_texture: Texture2D = preload("res://gdk-stoneblock/assets/images/sb_button/button_square_normal.png"):
	set(v):
		normal_texture = v
		if is_inside_tree(): _update_ui()
@export var hover_texture: Texture2D = preload("res://gdk-stoneblock/assets/images/sb_button/button_square_hover.png"):
	set(v):
		hover_texture = v
		if is_inside_tree(): _update_ui()
@export var pressed_texture: Texture2D = preload("res://gdk-stoneblock/assets/images/sb_button/button_square_pressed.png"):
	set(v):
		pressed_texture = v
		if is_inside_tree(): _update_ui()
...
func _process(_delta: float) -> void:
	...
	if not Engine.is_editor_hint():
		...
		# Mise Ã  jour du Shader (Transitions)
		if normal_texture:
...
func _on_self_resized() -> void:
	# Ne pas rappeler _update_ui ici : set_deferred(custom_minimum_size) dÃ©clenche resized
...
func _update_ui() -> void:
	...
	if _btn:
		...
		# On n'Ã©tire pas les tailles physiques si on est en "Pure Visual Scale"
		...
		var core_h: int = 0
		if not text.is_empty():
			# Padding via offsets Label/IcÃ´ne uniquement ; StyleBoxEmpty Ã  0 (Ã©vite double compte).
		...
		if normal_texture:
			...
			# Mise Ã  jour des paramÃ¨tres du shader
