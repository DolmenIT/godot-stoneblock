...
## Composant StoneBlock pour flouter progressivement l'Ã©cran.
## Placer en enfant de la Camera3D pour un meilleur contrÃ´le.

@export_group("Timing")
## Temps (en secondes) oÃ¹ le flou commence.
@export var start_time: float = 33.0
## Temps (en secondes) oÃ¹ le flou atteint son maximum.
@export var end_time: float = 35.0

@export_group("Blur Settings")
## IntensitÃ© maximale du flou (LOD du shader).
@export var max_blur: float = 2.5
## Index du CanvasLayer. 
## Entre 1 et 119 : L'effet est derriÃ¨re l'UI des dialogues (120).
...
func _ready() -> void:
	...
	# CrÃ©ation dynamique du CanvasLayer pour le post-processing
	_canvas_layer = CanvasLayer.new()
	_canvas_layer.name = "BlurCanvasLayer"
	_canvas_layer.layer = layer
	add_child(_canvas_layer)
	
	# CrÃ©ation du ColorRect qui servira de support au shader
...
func _process(delta: float) -> void:
	...
	if _elapsed_time < start_time:
		...
		# Interpolation linÃ©aire entre start_time et end_time
...
func _set_blur(amount: float) -> void:
	if _blur_rect and _blur_rect.material:
		...
		# Optimisation : masquer le rect si le flou est Ã  0
