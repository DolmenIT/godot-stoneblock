...
var ships_data = {
	"phantom_jet": {"name": "Phantom Jet", "price": 0, "color": Color.SKY_BLUE, "desc": "Vaisseau de base Ã©quilibrÃ©."},
	"nexus_disk": {"name": "Nexus Disk", "price": 250, "color": Color.SEA_GREEN, "desc": "Plus agile mais moins blindÃ©."},
...
var weapons_data = {
	"triple_shot": {"name": "Triple Shot", "price": 0, "color": Color.GOLD, "desc": "Tir en Ã©ventail standard."},
...
func _ready() -> void:
	if not SB_Core.instance:
		print("[Workshop] Erreur: SB_Core non trouvÃ©.")
		return
...
func _on_tab_changed(tab: String) -> void:
	current_tab = tab
	# Mise Ã  jour visuelle des onglets (optionnel: changer couleur)
	_rebuild_grid()
...
func _on_item_pressed(id: String, item: Dictionary) -> void:
	...
	if id in unlocked:
		...
		if coins >= item["price"]:
			...
			SB_Core.instance.set_stat(selected_key, id) # Auto-sÃ©lection aprÃ¨s achat
			SB_Core.instance.log_msg("Achat validÃ© : " + item["name"], "success")
		else:
