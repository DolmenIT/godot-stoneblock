...
## GÃ¨re l'Ã©jection initiale, la friction, le magnÃ©tisme vers le joueur et la collecte.
...
## ScÃ¨ne (glb/tscn) du visuel personnalisÃ© (ModÃ¨le 3D).
@export var visual_scene: PackedScene
## Ã‰chelle du modÃ¨le personnalisÃ©.
@export var visual_scale: float = 1.0
## Rotation corrective du modÃ¨le personnalisÃ©.
...
## CatÃ©gorie de flou pour cet objet (Layer 11 par dÃ©faut pour les loots).
...
func _refresh_visuals() -> void:
	# CrÃ©ation du pivot de rotation
	...
	# Si un visuel personnalisÃ© est spÃ©cifiÃ©
	if visual_scene:
		...
		# Forcer l'application du Bloom sur le nouveau modÃ¨le
...
func _apply_bloom_layers() -> void:
	# SÃ©curitÃ© : On dÃ©termine le numÃ©ro de layer explicitement
	var layer_num: int = 1
	...
	# CrÃ©ation du masque (Bit 1 pour la vue normale + Bit du Bloom choisi)
...
func _process(delta: float) -> void:
	...
	# Rotation appliquÃ©e au pivot s'il existe, sinon Ã  self
