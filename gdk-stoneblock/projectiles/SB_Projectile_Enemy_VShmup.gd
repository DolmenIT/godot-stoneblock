...
## â˜„ï¸ SB_Projectile_Enemy_VShmup : Projectile tirÃ© par les ennemis.
## Inflige des dÃ©gÃ¢ts au bouclier du joueur.

@export_group("VFX (Sprite Based)")
@export var bullet_color: Color = Color(1.0, 0.2, 0.4, 1.0) # Rose/Rouge par dÃ©faut
	...
	if _visual_node:
		for i in range(3):
			...
			if ghost is AnimatedSprite3D:
				ghost.autoplay = ""
			add_child(ghost)
			# Dupliquer le matÃ©riau APRÃˆS add_child
...
func _process(delta: float) -> void:
	...
	# Mise Ã  jour des fantÃ´mes (espacement doublÃ©)
