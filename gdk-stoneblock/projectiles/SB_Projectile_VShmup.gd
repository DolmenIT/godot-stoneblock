...
## GÃ¨re le mouvement, les trajectoires oscillantes et l'auto-destruction.

# --- Configuration ---
@export_group("Movement")
@export var speed: float = 50.0
@export var direction: Vector3 = Vector3(0, 0, -1) # Par dÃ©faut vers le haut en Top-Down
## DÃ©gÃ¢ts infligÃ©s Ã  l'impact (1.0 par dÃ©faut).
...
## Si activÃ©, le projectile s'ajuste en Y vers l'ennemi le plus proche dans la boÃ®te de dÃ©tection.
@export var auto_y_homing: bool = true
## Largeur totale (X) de la fenÃªtre de dÃ©tection.
@export var homing_width: float = 1.0
## Profondeur (Z) de la fenÃªtre de dÃ©tection devant le projectile.
@export var homing_depth: float = 10.0
## Hauteur totale (Y) de la fenÃªtre de dÃ©tection.
@export var homing_height: float = 25.0
## Vitesse Ã  laquelle le projectile se dÃ©place en Y vers sa cible.
@export var homing_y_speed: float = 12.0
## Affiche la boÃ®te de dÃ©tection (Cyan transparent) pour le debug.
@export var debug_show_homing_box: bool = false

@export_group("VFX (Sprite Based)")
@export var bullet_color: Color = Color(1.0, 0.8, 0.2, 1.0) # Jaune/Orange par dÃ©faut
@export var bullet_scale: float = 1.0

@export_group("Bloom SÃ©lectif")
## Si activÃ©, ajoute le render layer du bloom sur le visuel et les fantÃ´mes.
@export var use_bloom: bool = true

enum BloomCategory { LONG = 11, MEDIUM = 12, SHORT = 13 }
## CatÃ©gorie de flou (Rayon diffÃ©rent dans BloomConfig).
...
func _ready() -> void:
	...
	# DÃ©tection du visuel principal
	...
	if _visual_node:
		for i in range(3):
			...
			if ghost is AnimatedSprite3D:
				ghost.autoplay = ""
			add_child(ghost)
			# Dupliquer le matÃ©riau APRÃˆS add_child pour qu'il soit dans l'arbre
			if ghost.material_override:
				ghost.material_override = ghost.material_override.duplicate()
			_ghosts.append(ghost)
	
	# Appliquer les couleurs au prochain frame (nÅ“uds garantis dans l'arbre)
	call_deferred("_apply_vfx_settings")

func _create_debug_box() -> void:
	print("[ProjectileDebug] Tentative de crÃ©ation de la boÃ®te devant: ", direction)
	...
	mat.cull_mode = BaseMaterial3D.CULL_DISABLED # Visible de l'intÃ©rieur et de l'extÃ©rieur
	mesh_inst.material_override = mat
	
	add_child(mesh_inst)
	# On dÃ©cale la boÃ®te pour qu'elle soit DEVANT le projectile (milieu de la profondeur)
	mesh_inst.position = direction.normalized() * (homing_depth / 2.0)
	
	# Correction IP-078: Forcer les layers pour Ãªtre visible par la camÃ©ra du Shmup
...
func _apply_vfx_settings() -> void:
	...
	# FantÃ´mes avec transparence dÃ©gressive (60%, 30%, 10%)
	var alphas = [0.6, 0.3, 0.1]
	for i in range(_ghosts.size()):
		_set_node_color(_ghosts[i], bullet_color, alphas[i])
	
	# Bloom sÃ©lectif : ajouter le render layer aux visuels
	_apply_bloom_layers()
...
func _apply_bloom_layers() -> void:
	...
	if _visual_node and _visual_node is VisualInstance3D:
		...
		print("[Projectile] Bloom Layer appliquÃ©: %d (mask: %d)" % [int(bloom_category), bloom_mask])
	
	# FantÃ´mes (rÃ©manances)
...
func _update_movement(delta: float) -> void:
	# MÃ©moriser la position avant le mouvement
	_prev_pos = global_position
	
	# Mouvement de base (LinÃ©aire)
	var movement = direction * speed * delta
	global_position += movement
	
	# Oscillation (LatÃ©rale par rapport Ã  la direction)
	if use_oscillation:
	...
	# Mise Ã  jour des fantÃ´mes (position rÃ©elle, espacement doublÃ©)
...
func _check_cleanup() -> void:
	# Auto-destruction aprÃ¨s certain temps
	if _total_time > life_time:
		queue_free()
	
	# Auto-destruction aprÃ¨s certaine distance
...
func _apply_y_homing(delta: float) -> void:
	...
	for enemy in enemies:
		...
		# 1. VÃ©rification "Devant" (Dot product avec la direction)
		# On utilise la direction normalisÃ©e pour obtenir la distance projetÃ©e
		var dist_fwd = diff.dot(direction.normalized())
		
		# Si l'ennemi est derriÃ¨re ou trop loin devant
		if dist_fwd < 0 or dist_fwd > homing_depth: continue
		
		# 2. VÃ©rification LatÃ©rale et Verticale (BoÃ®te relative Ã  l'axe de tir)
		# On calcule l'Ã©cart perpendiculaire Ã  la trajectoire
		var lateral_offset = (diff - direction.normalized() * dist_fwd)
		
		if abs(lateral_offset.x) <= hw and abs(lateral_offset.y) <= hh:
			# PrioritÃ© Ã  l'objet le plus proche sur la trajectoire (Z-first)
