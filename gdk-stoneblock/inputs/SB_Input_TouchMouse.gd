...
## Nom de la mÃ©thode Ã  appeler sur la cible (ex: "_on_raycast_hit").
@export var target_method: String = "_on_raycast_hit"
## Nom de la mÃ©thode de tir Ã  appeler sur la cible (ex: "set_firing").
@export var fire_method: String = "set_firing"

@export_group("Configuration")
## Hauteur Y du plan virtuel de dÃ©tection.
@export var plane_height: float = 0.0
## Si vrai, traite les clics souris comme des inputs de dÃ©placement.
@export var use_mouse: bool = true
## Si vrai, traite les Ã©vÃ©nements tactiles comme des inputs de dÃ©placement.
@export var use_touch: bool = true

@export_group("Camera")
## Nom de la camÃ©ra Ã  utiliser pour le raycast (si vide, cherche "Mainground_Camera").
@export var camera_name: String = "Mainground_Camera"

@export_group("Debug Visuals")
## Si vrai, affiche un plan semi-transparent pour visualiser la zone de dÃ©tection.
...
# Cache d'Ã©tats (Inspiration SB_Input_Gamepad)
...
func _find_references() -> void:
	# Recherche de la camÃ©ra
	if camera_name != "":
		# On cherche d'abord dans la scÃ¨ne courante (via owner ou scÃ¨ne active)
		var root = owner
	...
	if target_name != "":
		...
		if root:
			...
			if _target_cached:
				# print("[TouchMouse] Cible trouvÃ©e: ", _target_cached.name)
...
func _process(_delta: float) -> void:
	...
	if _is_dragging:
		...
		# On arrÃªte le mouvement pilotÃ© par cible dÃ¨s qu'on relÃ¢che
		_stop_movement()
	
	# Tentative de rÃ©cupÃ©ration de la cible si elle est absente (IP-037)
	# Utile lors du chargement asynchrone oÃ¹ le joueur est instanciÃ© plus tard.
	if not _target_cached and not Engine.is_editor_hint():
		# On ne le fait pas Ã  chaque frame, mais par exemple une fois par seconde
...
func _process_input(_unused_pos: Vector2) -> void:
	...
	# On utilise la position de souris relative au viewport de la camÃ©ra 
	# pour Ã©viter les dÃ©calages de coordonnÃ©es entre Root et SubViewport.
...
func _stop_movement() -> void:
	...
	# On appelle explicitement l'arrÃªt du mouvement pour annuler le flag _has_target_pos
...
func _calculate_raycast(screen_pos: Vector2) -> Vector3:
	# Utilisation des mÃ©thodes natives (Fonctionne pour Ortho et Perspective)
...
func _update_debug_plane() -> void:
	...
	if not _debug_mesh:
		...
		# On attache le mesh au parent de la camÃ©ra pour qu'il soit dans le bon SubViewport
		if _camera_cached:
	...
	# Gestion du changement de parent si nÃ©cessaire (en cas de re-find de camÃ©ra)
	...
	# Adaptation Ã  la taille de la camÃ©ra
	...
	if _camera_cached:
		if _camera_cached.projection == Camera3D.PROJECTION_ORTHOGONAL:
			...
			# Perspective : on laisse la taille manuelle ou on pourrait calculer le frustum Ã  plane_height
