...
## Permet de dÃ©finir un template commun (modÃ¨le, PV, tir) et de gÃ©nÃ©rer la formation.
...
## La scÃ¨ne d'ennemi Ã  instancier (par dÃ©faut SB_Enemy_VShmup).
@export var enemy_scene: PackedScene = preload("res://gdk-stoneblock/enemies/SB_Enemy_VShmup.tscn")
## Remplacer le modÃ¨le 3D de tous les ennemis du groupe.
@export var vessel_scene: PackedScene :
	set(v):
		vessel_scene = v
		_sync_children()
## Ã‰chelle des visuels d'ennemis.
...
@export var fire_interval: float = 1.6 :
	set(v):
		fire_interval = v
		_sync_children()
## Si VRAI, le groupe Ã©crase les PV, le modÃ¨le et la cadence de tir des enfants.
## Si FAUX, le groupe respecte les rÃ©glages individuels des Prefabs.
...
## Nombre d'ennemis Ã  gÃ©nÃ©rer dans ce groupe.
@export var enemy_count: int = 3
## Bouton : Cliquer ici (cocher puis dÃ©cocher) pour recrÃ©er le groupe d'ennemis.
...
## Amplitude du mouvement latÃ©ral (sinusoÃ¯de). 0 = DÃ©sactivÃ©.
@export var wave_amplitude: float = 8.0
## Vitesse du mouvement latÃ©ral.
@export var wave_speed: float = 2.0

@export_group("Activation")
## Distance (Z) Ã  partir de laquelle le groupe entier s'active.
...
func _generate_enemies() -> void:
	...
	for i in range(enemy_count):
		...
		# IMPORTANT : Pour que les enfants soient sauvÃ©s dans la scÃ¨ne (.tscn)
...
func _sync_children() -> void:
	if not override_prefab_stats: 
		# On force quand mÃªme le 'follow_group' pour que le mouvement fonctionne
		for child in get_children():
