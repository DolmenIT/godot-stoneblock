...
func _ready() -> void:
	...
	# On cherche les SB_Enemy_VShmup qui ne sont pas nous-mÃªmes
	_protector_count = 0
	...
	# VÃ©rification rÃ©cursive dans les sous-nÅ“uds si nÃ©cessaire (Optionnel)
	# Pour l'instant on s'en tient aux enfants directs pour la clartÃ©.
...
func _on_protector_destroyed(_pos: Vector3) -> void:
	...
	if _protector_count <= 0:
		...
		# On pourrait ajouter un petit effet sonore ou visuel de dÃ©verrouillage ici
...
func _shield_flash() -> void:
	# Feedback visuel de bouclier (Cyan/Bleu) pour indiquer l'indestructibilitÃ©
