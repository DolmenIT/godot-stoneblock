@tool
@icon("res://gdk-stoneblock/assets/icons/SB_Core.svg")
extends SB_Config
class_name SB_GameDatas

## 💾 SB_GameDatas : Gestionnaire de progression persistant du joueur.
## Stocke l'or, l'inventaire et les choix du joueur dans un fichier JSON.

# --- AccÃ¨s Statique (Singleton) ---
static var instance: SB_GameDatas

signal gold_changed(new_amount: int)
signal inventory_updated(data: Dictionary)
signal data_loaded()

const DEFAULT_SAVE_PATH = "user://game_progress.json"

@export_group("Configuration")
## Montant d'or initial (ou pour le debug).
@export var debug_gold: int = 1000

@export_group("Debug")
## Forcer une rÃ©initialisation des donnÃ©es au prochain chargement.
@export var force_reset: bool = false

# --- DonnÃ©es de Jeu ---
var data: Dictionary = {
	"gold": 1000, # Un peu d'or par dÃ©faut pour les tests
	"inventory": {
		"Scout MK-1": { "rarity": 0, "xp": 0, "stats_bonus": 1.0 }
	},
	"selected_ship_id": "Scout MK-1",
	"armory": {
		"weapons": {
			"rapid_fire":    { "unlocked": false, "rarity": 0, "xp": 0, "stats_bonus": 1.0 },
			"split_shot":    { "unlocked": false, "rarity": 0, "xp": 0, "stats_bonus": 1.0 },
			"multiple_shot": { "unlocked": false, "rarity": 0, "xp": 0, "stats_bonus": 1.0 },
			"oriented_shot": { "unlocked": false, "rarity": 0, "xp": 0, "stats_bonus": 1.0 },
			"rotative_shot": { "unlocked": false, "rarity": 0, "xp": 0, "stats_bonus": 1.0 }
		},
		"ammos": {
			"beam_shot":      { "unlocked": false, "rarity": 0, "xp": 0, "stats_bonus": 1.0 },
			"ricochet_shot":  { "unlocked": false, "rarity": 0, "xp": 0, "stats_bonus": 1.0 },
			"explosive_shot": { "unlocked": false, "rarity": 0, "xp": 0, "stats_bonus": 1.0 },
			"homing_shot":    { "unlocked": false, "rarity": 0, "xp": 0, "stats_bonus": 1.0 },
			"laser_shot":     { "unlocked": false, "rarity": 0, "xp": 0, "stats_bonus": 1.0 }
		}
	},
	"last_save_date": ""
}

func _enter_tree() -> void:
	instance = self
	if SB_Core.instance:
		SB_Core.instance.log_msg("Manager GameDatas initialisÃ©.", "success")

func _ready() -> void:
	if Engine.is_editor_hint(): return
	
	if force_reset:
		_initialize_defaults()
		save_to_disk()
		force_reset = false
	else:
		load_from_disk()

# â”€â”€ API Publique â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

## VÃ©rifie si le joueur peut payer un montant.
func can_afford(amount: int) -> bool:
	return data.get("gold", 0) >= amount

## DÃ©pense de l'or si possible. Retourne vrai si la transaction a rÃ©ussi.
func spend_gold(amount: int) -> bool:
	if can_afford(amount):
		add_gold(-amount)
		return true
	return false

## RÃ©cupÃ¨re une valeur de la sauvegarde.
func get_value(key: String, default = null):
	return data.get(key, default)

## Modifie une valeur et sauvegarde automatiquement.
func set_value(key: String, value, auto_save: bool = true) -> void:
	data[key] = value
	if key == "gold": gold_changed.emit(value)
	if auto_save: save_to_disk()

## Ajoute ou retire de l'or.
func add_gold(amount: int) -> void:
	var current_gold = data.get("gold", 0)
	set_value("gold", current_gold + amount)

## DÃ©bloque un vaisseau.
func unlock_ship(ship_id: String) -> void:
	var inventory = get_value("inventory", {})
	if not inventory.has(ship_id):
		inventory[ship_id] = { "rarity": 0, "xp": 0, "stats_bonus": 1.0 }
		set_value("inventory", inventory)
		inventory_updated.emit(data)

func unlock_armory_item(category: String, item_id: String) -> void:
	var armory = get_value("armory", {})
	var cat_key = "weapons" if category == "weapon" else "ammos"
	if armory.has(cat_key) and armory[cat_key].has(item_id):
		armory[cat_key][item_id]["unlocked"] = true
		set_value("armory", armory)
		inventory_updated.emit(data)

## RÃ©cupÃ¨re les stats d'un item spÃ©cifique (vaisseau, arme ou munition).
func get_item_stats(category: String, item_id: String) -> Dictionary:
	var root = data.get("inventory", {}) if category == "ship" else data.get("armory", {}).get(category + "s", {})
	var stats = root.get(item_id, { "rarity": 0, "xp": 0, "stats_bonus": 1.0 }).duplicate()
	
	# Injection de sÃ©curitÃ© si le champ 'unlocked' manque (vieille sauvegarde)
	if not stats.is_empty() and not stats.has("unlocked"):
		if item_id == "Scout MK-1":
			stats["unlocked"] = true
		else:
			stats["unlocked"] = false
			
	return stats

## Ajoute de l'XP Ã  un item (+10% stats par palier).
func add_item_xp(category: String, item_id: String, amount: int = 20) -> void:
	var root = data.get("inventory") if category == "ship" else data.get("armory", {}).get(category + "s")
	if root and root.has(item_id):
		var item = root[item_id]
		item["xp"] = clampi(item.get("xp", 0) + amount, 0, 100)
		item["stats_bonus"] = item.get("stats_bonus", 1.0) + 0.1
		set_value("inventory" if category == "ship" else "armory", data["inventory"] if category == "ship" else data["armory"])

## Augmente la raretÃ© d'un item.
func promote_item(category: String, item_id: String) -> void:
	var root = data.get("inventory") if category == "ship" else data.get("armory", {}).get(category + "s")
	if root and root.has(item_id):
		var item = root[item_id]
		item["rarity"] = clampi(item.get("rarity", 0) + 1, 0, 2)
		item["xp"] = 0 # On reset l'XP pour le prochain palier
		set_value("inventory" if category == "ship" else "armory", data["inventory"] if category == "ship" else data["armory"])

# â”€â”€ Gestion du Disque â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

## Sauvegarde les donnÃ©es actuelles sur le disque.
func save_to_disk(target_path: String = DEFAULT_SAVE_PATH) -> void:
	data["last_save_date"] = Time.get_datetime_string_from_system()
	
	var file = FileAccess.open(target_path, FileAccess.WRITE)
	if file:
		var json_string = JSON.stringify(data, "\t") # Joli formatage pour le debug
		file.store_string(json_string)
		file.close()
		if SB_Core.instance:
			SB_Core.instance.log_msg("DonnÃ©es de jeu sauvegardÃ©es : " + target_path.get_file(), "success")
	else:
		if SB_Core.instance:
			SB_Core.instance.log_msg("Erreur : Impossible d'Ã©crire le fichier de sauvegarde !", "error")

## Charge les donnÃ©es depuis le disque.
func load_from_disk(target_path: String = DEFAULT_SAVE_PATH) -> void:
	if not FileAccess.file_exists(target_path):
		if SB_Core.instance: SB_Core.instance.log_msg("Aucune sauvegarde trouvÃ©e. Initialisation par dÃ©faut.", "info")
		_initialize_defaults()
		save_to_disk(target_path)
		return

	var file = FileAccess.open(target_path, FileAccess.READ)
	if file:
		var json_string = file.get_as_text()
		var json = JSON.new()
		var parse_result = json.parse(json_string)
		
		if parse_result == OK:
			var loaded_data = json.get_data()
			if loaded_data is Dictionary:
				# On fusionne avec les clÃ©s par dÃ©faut pour ne rien perdre si on ajoute des variables au script
				for key in loaded_data:
					data[key] = loaded_data[key]
				
				# Migration d'ancienne structure : owned_ships -> inventory
				if data.has("owned_ships") and not data.has("inventory"):
					var inv = {}
					for s in data["owned_ships"]:
						inv[s] = { "rarity": 0, "xp": 0, "stats_bonus": 1.0 }
					data["inventory"] = inv
					data.erase("owned_ships")
				
				if SB_Core.instance:
					SB_Core.instance.log_msg("DonnÃ©es de jeu chargÃ©es (%s)." % target_path.get_file(), "success")
				data_loaded.emit()
		file.close()

## Importe et fusionne des donnÃ©es d'un autre fichier (Ex: DLC, Mission spÃ©ciale, etc.)
func import_and_merge(path: String) -> void:
	if not FileAccess.file_exists(path): return
	
	var file = FileAccess.open(path, FileAccess.READ)
	if file:
		var json_string = file.get_as_text()
		var json = JSON.new()
		if json.parse(json_string) == OK:
			var external_data = json.get_data()
			if external_data is Dictionary:
				for key in external_data:
					# Logique de fusion spÃ©cifique
					if key == "gold":
						data[key] += external_data[key]
					elif key == "inventory":
						var inv = data.get("inventory", {})
						var ext_inv = external_data[key]
						for s in ext_inv:
							if not inv.has(s): inv[s] = ext_inv[s]
					else:
						data[key] = external_data[key]
				save_to_disk() # On persiste la fusion
		file.close()

# â”€â”€ Internes â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

func _initialize_defaults() -> void:
	data = {
		"gold": debug_gold,
		"inventory": {
			"Scout MK-1": { "rarity": 0, "xp": 0, "stats_bonus": 1.0 }
		},
		"selected_ship_id": "Scout MK-1",
		"armory": {
			"weapons": {
				"rapid_fire":    { "unlocked": false, "rarity": 0, "xp": 0, "stats_bonus": 1.0 },
				"split_shot":    { "unlocked": false, "rarity": 0, "xp": 0, "stats_bonus": 1.0 },
				"multiple_shot": { "unlocked": false, "rarity": 0, "xp": 0, "stats_bonus": 1.0 },
				"oriented_shot": { "unlocked": false, "rarity": 0, "xp": 0, "stats_bonus": 1.0 },
				"rotative_shot": { "unlocked": false, "rarity": 0, "xp": 0, "stats_bonus": 1.0 }
			},
			"ammos": {
				"beam_shot":      { "unlocked": false, "rarity": 0, "xp": 0, "stats_bonus": 1.0 },
				"ricochet_shot":  { "unlocked": false, "rarity": 0, "xp": 0, "stats_bonus": 1.0 },
				"explosive_shot": { "unlocked": false, "rarity": 0, "xp": 0, "stats_bonus": 1.0 },
				"homing_shot":    { "unlocked": false, "rarity": 0, "xp": 0, "stats_bonus": 1.0 },
				"laser_shot":     { "unlocked": false, "rarity": 0, "xp": 0, "stats_bonus": 1.0 }
			}
		},
		"last_save_date": Time.get_datetime_string_from_system()
	}
	gold_changed.emit(data["gold"])
	inventory_updated.emit(data)
