...
## D̩finit l'intensit̩ et la couleur du flash pour une instance (IP-115).
...
func set_instance_flash(obj: Node3D, intensity: float, color: Color = Color.WHITE) -> void:
	...
	var data = _registry[id]
	# On stocke l'intensit̩ dans le canal R de la custom data (Color est utilis̩ comme vec4)
...
func _resize_multimesh(mm_inst: MultiMeshInstance3D, new_size: int) -> void:
	...
	# En Godot 4, changer instance_count sur un MultiMesh pr̩serve les donn̩es existantes.
	# Mais on doit s'assurer que les nouvelles instances sont "cach̩es" (scale 0)
