...
## Teinte du bouton en Ã©tat normal.
...
@export_group("Ã‰clairage & Bloom")
## IntensitÃ© de l'Ã©mission lumineuse en Ã©tat normal.
@export var emission_energy_normal: float = 0.0
## IntensitÃ© de l'Ã©mission lumineuse lors du survol.
@export var emission_energy_hover: float = 2.0
## IntensitÃ© de l'Ã©mission lumineuse lors du clic.
@export var emission_energy_pressed: float = 1.0

@export_subgroup("Layers (Cull Masks)")
## Calque par dÃ©faut (souvent 1 + un calque de bloom).
...
## Taille du mesh (fond) du bouton (ex: (0.3, 0.1) pour rectangle, (0.3, 0.3) pour carrÃ©).
@export var mesh_size: Vector2 = Vector2(0.3, 0.1)
## Ã‰chelle de base du bouton dans l'espace 3D.
