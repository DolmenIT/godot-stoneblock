...
## GÃ¨re le mouvement, l'inclinaison (banking) et les limites de l'Ã©cran.
...
@export var max_bank_angle: float = 25.0 # DegrÃ©s
...
## Multiplicateur de vitesse lors de l'accÃ©lÃ©ration (Boost).
...
## CoÃ»t en Ã©nergie par seconde lors du Boost.
@export var energy_cost_boost: float = 10.0

@export_subgroup("Reactors Visuals")
## Nombre de rÃ©acteurs (1 = centre, 2+= symÃ©trie).
@export var engine_count: int = 2:
	set(v):
		engine_count = v
		if Engine.is_editor_hint(): _setup_engines()
## Ã‰cartement horizontal entre les rÃ©acteurs.
@export var engine_spacing_x: float = 0.5:
	set(v):
		engine_spacing_x = v
		if Engine.is_editor_hint(): _setup_engines()
## DÃ©calage vertical des rÃ©acteurs.
@export var engine_offset_y: float = 0.1:
	set(v):
		engine_offset_y = v
		if Engine.is_editor_hint(): _setup_engines()
## DÃ©calage en profondeur des rÃ©acteurs (Z+ = ArriÃ¨re).
@export var engine_offset_z: float = 1.2:
	set(v):
		engine_offset_z = v
		if Engine.is_editor_hint(): _setup_engines()
## Positions locales manuelles (DÃ©passe les rÃ©glages ci-dessus si renseignÃ©).
@export var manual_engine_positions: Array[Vector3] = []
## ScÃ¨ne de particules pour les rÃ©acteurs.
@export var engine_particle_scene: PackedScene = preload("res://gdk-stoneblock/effects/SB_EngineParticles.tscn")
## Ã‰nergie maximum du vaisseau.
@export var energy_max: float = 100.0
## Vitesse de rÃ©gÃ©nÃ©ration d'Ã©nergie (unitÃ©s par seconde).
...
## Bouclier maximum (se rÃ©gÃ©nÃ¨re au repos).
@export var shield_max: float = 5.0
## Temps d'attente sans dÃ©gÃ¢ts avant rÃ©gÃ©nÃ©ration (secondes).
@export var shield_regen_delay: float = 2.0
## Vitesse de rÃ©gÃ©nÃ©ration du bouclier (points par seconde).
@export var shield_regen_speed: float = 5.0
## Temps d'invulnÃ©rabilitÃ© aprÃ¨s un impact (secondes).
@export var invul_duration: float = 2.0

@export_group("Combat")
@export var projectile_scene: PackedScene = preload("res://gdk-stoneblock/projectiles/SB_Projectile_VShmup.tscn")
...
## ScÃ¨ne du projectile Ã  utiliser pour le Triple Shot (optionnel, sinon utilise le dÃ©faut).
@export var triple_projectile_scene: PackedScene = null
## Angle d'Ã©ventail pour le Triple Shot (degrÃ©s).
@export var triple_shot_spread: float = 15.0

@export var explosion_scene: PackedScene = preload("res://gdk-stoneblock/effects/SB_PlayerExplosion_VShmup.tscn")
## ModÃ¨le 3D du vaisseau (ScÃ¨ne GLB/TSCN). Si dÃ©fini, remplace le visuel par dÃ©faut.
...
func _ready() -> void:
	...
	# Instanciation dynamique du vaisseau si spÃ©cifiÃ©
	if vessel_scene:
		_apply_vessel_visuals()
		
	_setup_engines()

## Configure le vaisseau et les power-ups selon les sÃ©lections du Workshop.
func apply_workshop_settings(ship_id: String, powerup_id: String) -> void:
	...
	match ship_id:
		"phantom_jet":
			movement_speed = 30.0
			health_max = 100.0
			# ModÃ¨le par dÃ©faut dÃ©jÃ  chargÃ© via l'export ou mainground.tscn
		"nexus_disk":
	...
	health = health_max
	if SB_Core.instance:
		SB_Core.instance.log_msg("Workshop : Configuration appliquÃ©e pour " + ship_id, "info")
	
	# 2. Gestion du Power-up de dÃ©part
	match powerup_id:
		"triple_shot":
...
func _apply_vessel_visuals() -> void:
	...
	# CrÃ©ation d'un pivot pour sÃ©parer la rotation corrective du banking technique
...
func _setup_engines() -> void:
	...
	# Nettoyage des anciens rÃ©acteurs
...
func _process(delta: float) -> void:
	...
	# Mise Ã  jour des timers de dash
	# --- Timers compensÃ©s ---
	...
	# RÃ©gÃ©nÃ©ration d'Ã©nergie (On garde peut-Ãªtre le temps ralenti pour l'Ã©nergie ? Non, on va dire que le gÃ©nÃ©rateur du vaisseau est aussi en temps rÃ©el)
	energy = min(energy_max, energy + energy_regen * effective_delta)
	
	# RÃ©gÃ©nÃ©ration de bouclier
	...
	if _triple_shot_timer > 0.0:
		...
		if _triple_shot_timer <= 0:
			...
			if SB_Core.instance:
				SB_Core.instance.log_msg("Triple Shot expirÃ© !", "info")
...
func _process_movement(delta: float) -> void:
	...
	if is_direct_input or is_dashing:
		...
		# Propulsion (AccÃ©lÃ©ration / Freinage)
		var thrust = 0.0 
		...
		# On clampe la cible pour qu'elle ne dÃ©passe pas les limites du vaisseau
		# Cela Ã©vite que le vaisseau ne "pousse" indÃ©finiment contre les bords
		...
		if dir_3d.length() > 0.1:
			...
			# Banking simulÃ© basÃ© sur la direction du mouvement
...
func _process_visuals(delta: float) -> void:
	...
	if not mesh:
		# Fallback : Prend le premier enfant qui ressemble Ã  un visuel
	...
	# Inclinaison latÃ©rale (Banking lÃ©ger)
...
func set_dash(active: bool) -> void:
	...
	# VÃ©rification Ã©nergie
	if energy < energy_cost_dash: return
	
	# RÃ©cupÃ©ration de la direction actuelle
...
func die() -> void:
	...
	# DÃ©sactivation visuelle robuste
	_hide_ship()
...
## API PUBLIQUE : Gestion DÃ©gÃ¢ts & Bouclier
func take_damage(amount: float) -> void:
	...
	# Puis la vie si reste des dÃ©gÃ¢ts
	if amount > 0:
		health -= amount
	...
	if health <= 0:
		...
		if SB_Core.instance:
			SB_Core.instance.log_msg("DÃ©gÃ¢ts reÃ§us ! Vie : %d%%, Bouclier : %d%%" % [(health/health_max)*100, (shield/shield_max)*100], "warning")
...
func add_coin(amount: float) -> void:
	...
	if SB_Core.instance:
		...
		SB_Core.instance.log_msg("Coin collectÃ© : +%d" % amount, "success")
...
func _fire_triple_shot() -> void:
	energy -= energy_cost_fire * 2.0 # CoÃ»t lÃ©gÃ¨rement plus Ã©levÃ©
...
func _spawn_projectile(bullet: Node3D, pos: Vector3, dir: Vector3) -> void:
	# On cherche un pivot de dÃ©filement mondial (Z-only) pour garder la portÃ©e stable sans dÃ©viation en X
...
## API PUBLIQUE : ContrÃ´le de l'UI et des actions
func set_controls_enabled(v: bool) -> void:
	controls_enabled = v
	if SB_Core.instance:
		SB_Core.instance.log_msg("ContrÃ´les du joueur : " + ("Actifs" if v else "DÃ©sactivÃ©s"), "info")
